					...
					else if ( message_type == LENS_PLAYERADD ) /*  New User logged in, add local player  */
					{
						if (net_id != pid)
						{
							...
							if ( client_entity.length() )
							{
								...
								if ( !player_entity )
								{
									lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Error Creating Network Client Entity " << client_entity << std::endl;
								}
								else
								{
...
bool pc_network_init()
{
	...
	if ( client == nullptr )
	{
		lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ":" << __LINE__ << " | An error occurred while trying to create an ENet client host." << std::endl;
		enet_deinitialize();
		return net_active;
	}

	/* Connect to some.server.net:8714. */
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ":" << __LINE__ << " | Connecting to " << server_ip << std::endl;
	...
	if ( enet_host_service(client, &event, 5000) > 0 && event.type == ENET_EVENT_TYPE_CONNECT )
	{
		if ( enet_host_service(client, &event, 5000) > 0 && event.type == ENET_EVENT_TYPE_RECEIVE )
		{
			...
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ":" << __LINE__ << " | Connection to " << server_ip << " succeeded." << std::endl;
			net_active = true;
			sdlcore_thread = SDL_CreateThread( pc_network_thread, nullptr );
		}
	}
	else
	{
		enet_peer_reset( peer );
		lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ":" << __LINE__ << " | Connection to " << server_ip << " failed." << std::endl;
	}
	return net_active;
}
