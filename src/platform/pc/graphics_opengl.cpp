...
LUX_DISPLAY_FUNCTION bool Lux_OGL_Init( uint16_t  width, uint16_t height, uint8_t bpp, uint16_t * actual_width, uint16_t * actual_height )
{
	...
	{
		lux::core->SystemMessage(__FILE__, __LINE__, SYSTEM_MESSAGE_LOG) << " Couldn't set "
	...
	{
		lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "256x256 icon not found." << std::endl;
	...
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Video Driver: OpenGL " << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Vendor: " << glGetString(GL_VENDOR) << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Renderer: " << glGetString(GL_RENDERER) << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Version: " << glGetString(GL_VERSION) << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Format: " << opengl_graphic_screen->w << "x" << opengl_graphic_screen->h << "x" << (int)opengl_graphic_screen->format->BitsPerPixel << "bpp" << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | Pointer Offset: " << opengl_graphic_ratio_width << "x" << opengl_graphic_ratio_height << std::endl;
	lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << __FILE__ << ": | GL_ARB_texture_non_power_of_two: " << Lux_OGL_QueryExtension((char *)"GL_ARB_texture_non_power_of_two") << std::endl;
