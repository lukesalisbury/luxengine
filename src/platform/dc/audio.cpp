...
int32_t AudioSystem::PlayEffect ( std::string requestSound, int32_t x, int32_t y )
{
	...
	if ( sample )
	{
		...
		{
			lux::core->SystemMessage(SYSTEM_MESSAGE_ERROR, __FILE__ , __LINE__) << " | Playing: " << requestSound  << " Length: " << len << std::endl;
...
int32_t AudioSystem::PlayMusic ( std::string requestMusic, int32_t loop, int32_t fadeLength )
{
	...
	lux::core->SystemMessage(SYSTEM_MESSAGE_ERROR, __FILE__ , __LINE__) << " | PlayMusic Error: '" << requestMusic << "' " << Mix_GetError()  << std::endl;
	return 0;
}
