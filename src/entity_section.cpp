...
void EntitySection::Switch(fixed x, fixed y)
{
	if ( this->parent )
	{
		std::cout << __FILE__ << ":" << __LINE__ << " | Section Entity Switch()" << std::endl;
		this->parent->Update();
	}
	...
	if ( this->children.size() )
	{
		...
		while( iter != this->children.end() )
		{
			std::cout << __FILE__ << ":" << __LINE__ << " | Entity " << (*iter)->id << " Switch()" << std::endl;
			(*iter)->x += x;
			(*iter)->y += y;
			(*iter)->Update();
			iter++;
...
{
	std::cout << " < EntitySection Close " << this->map_id << std::endl;
	if ( this->parent )
	{
		std::cout << __FILE__ << ":" << __LINE__ << " | Section Entity Close()" << std::endl;
		this->parent->Close();
	}
	...
	if ( this->children.size() )
	{
		...
		while( iter !=  this->children.end() )
		{
			std::cout << __FILE__ << ":" << __LINE__ << " | Entity " << (*iter)->id << " Close()" << std::endl;
			if ( (*iter)->deleted )
			{
				delete (*iter);
				iter = this->children.erase(iter);
