...
namespace egl_display {
	...
	bool open(uint32_t window_width, uint32_t window_height, bool window_fullscreen, uint8_t colour_depth)
	{
		...
		{
			TestEGLError( );
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to initialise EGL display. a" << std::endl;
			return false;
		}
		...
		{
			TestEGLError( );
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to find a config for EGL " << numConfigs << std::endl;
			return 0;
		}
		lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to find a config for EGL " << numConfigs << std::endl;
		*/
		...
		{
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to create GLES context!" << eglGetError() << std::endl;
			return 0;
		}
		...
		*/
		...
		if ( egl_surface == EGL_NO_SURFACE )
		{
			...
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to create EGL surface!" << std::endl;
			return 0;
		}
		...
		{
			lux::core->SystemMessage(SYSTEM_MESSAGE_LOG) << "Unable to make GLES context current" << std::endl;
			return 0;
		}
