...
/* modified */
  ...
  if ( f == NULL )
  {
    fprintf(stderr, "Function not found: %s\n", GETENTRYNAME(hdr,func) );
    return AMX_ERR_NOTFOUND;
  }
  else if ( amx->flags & AMX_FLAG_WATCH )
  {
	  fprintf(stdout, "Function Watch: %s\n", GETENTRYNAME(hdr,func) );
  }
...
/* modified */
  ...
  *result = f(amx,params);
  ...
  {
	  fprintf(stdout, "Function Error: %s\n", GETENTRYNAME(hdr,func) );
  }
  /* modified */
  return amx->error;
}
...
int AMXAPI amx_Exec(AMX *amx, cell *retval, int index)
{
  ...
  #if defined ASM32 && defined JIT
    if ((amx->flags & AMX_FLAG_JITC)!=0)
      i = amx_exec_jit(amx,retval,(char*)data);
    else
      i = amx_exec_asm(amx,retval,(char*)data);
  #elif defined ASM32
    i = amx_exec_asm(amx,retval,(char*)data);
  #else
    i = amx_exec_jit(amx,retval,(char*)data);
