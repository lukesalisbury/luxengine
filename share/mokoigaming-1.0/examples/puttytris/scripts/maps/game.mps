...
DrawField()
{
	...
	while (  x < 192  )
	{
		...
		{
			if ( grid[x/16][y/16] ) 
				GraphicsDraw(image[grid[x/16][y/16]-1], SPRITE, x+grid_area[0], y+grid_area[1], 4000, 0, 0);
			y += 16;
		}
		x += 16;
	}
...
HandleGame()
{
	...
	if ( clear_timer != 0)
	{
		...
		{
			if ( clear_lines[q] != -1 )
				GraphicsDraw("", RECTANGLE, grid_area[0] , grid_area[1] + (clear_lines[q] *16), 5000, grid_area[2] , 16, clear_colours[(clear_timer/100)]);
			q++;
		}
...
public Init(...)
{
	...
	scoreobj[0] = ObjectCreate("0", TEXT, 200, 408, 6000, 0, 0, 0xFF0000FF);
	scoreobj[1] = ObjectCreate("0", TEXT, 200, 432, 6000, 0, 0, 0xFF0000FF);
	scoreobj[2] = ObjectCreate("1", TEXT, 200, 456, 6000, 0, 0, 0xFF0000FF);
